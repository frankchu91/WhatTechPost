...
cover_image: https://raw.githubusercontent.com/frankchu91/WhatTechPost/main/assets/2026-08-17-bedrock.png
---
...
Closing it to new customers is the load-bearing half of the announcement. A platform that still wants a product to grow does not wall off its intake. That decision says the roadmap now lives somewhere else — in this case a new service, Amazon Bedrock AgentCore — and everything on the old one is in maintenance from here. The proof is in a quiet line of the maintenance notice: the model catalog is frozen, so any foundation model AWS adds after July 30 is AgentCore-only. The new toys ship to the new house. (Bedrock Agents wasn't alone, either — Kendra and Q Business entered maintenance mode in the same wave.)

![The platform-deprecation ladder: 1) renamed Classic, 2) closed to new customers — where Bedrock Agents is now, 3) catalog frozen, 4) end-of-life someday. Watch the intake, not the shutdown date.](https://raw.githubusercontent.com/frankchu91/WhatTechPost/main/assets/2026-08-17-bedrock-ladder.png)
...
AgentCore makes this concrete. It isn't a renamed Bedrock Agents — AWS rebuilt it around a different model: systems of agents that share memory, delegate to each other, and route through a unified gateway, where the old service was a single agent with basic tool calling. That's a better design. It's also a completely different set of abstractions, which means porting your Bedrock Agents logic to it isn't a migration, it's a rewrite. The upgrade and the rewrite are the same event.
